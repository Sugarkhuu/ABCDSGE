...
function Z = aux_stat(data, realdata, params)
	...
	if check_bad_data(data) || sum(any(data<=0))
		...
		if check_bad_data(checkme)
            ...
            pdm(4,:) = mean(e);
            pdm(5,:) = mean(e.^2 - 1);
            pdm(6,:) = mean(e(1:end-1,:).*e(2:end,:));
            pdm(7,:) = mean(e.*e1); % no cross correlation between errors
            Z = [Z;pdm];
		endif
	endif
	if bad_data
		Z = -1000*ones(7 + 5 + 5  + 5 + 15 + 3+7,1);
	endif
endfunction
